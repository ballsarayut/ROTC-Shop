...
async function run() {
  ...
  for (const p of dProducts) if (p.name) productsMap.set(p.name.trim(), p);
  ...
  for (const o of orders) {
     if (Number(o.totalAmount) === 2390 || Number(o.subtotal) === 2390) {
         let currentItems = [];
         try { currentItems = JSON.parse(o.items); } catch (e) { }
         ...
         if (!Array.isArray(currentItems) || currentItems.length < 10) {
            ...
            for (const item of newItems) {
                if (item.name.includes("ชุดฝึก") || item.name.includes("รองเท้า")) {
                    item.size = size; item.gender = gender;
                }
            }
            const payloadObj = { ...o };
            payloadObj.items = JSON.stringify(newItems);
            Object.keys(payloadObj).forEach(key => {
                if (typeof payloadObj[key] === 'object') payloadObj[key] = JSON.stringify(payloadObj[key]);
            });
  ...
  console.log("Orders to restore remaining:", toUpdate.length);
  const chunks = [];
  for(let i=0; i<toUpdate.length; i+=5) {
      chunks.push(toUpdate.slice(i, i+5));
  }
  ...
  for (const chunk of chunks) {
      await Promise.all(chunk.map(payloadObj => {
          ...
          }).then(res => res.text()).then(t => console.log("Updated", payloadObj.id, t));
      }));
  }
